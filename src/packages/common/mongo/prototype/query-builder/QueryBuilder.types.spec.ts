...
import { z, type MongoObject } from '../schema-v2';
...
jest.mock('../connection', () => {
  return {
    Connection: jest.fn().mockImplementation(() => {
      return {
        createModel: (name: string, schema: MongoObject<any>) => ({
...
describe('QueryBuilder:types', () => {
  describe('schema1', () => {
    ...
    const query = () => {
      const db = new connection.Connection('mongodb://127.0.0.1:27017/test');
    ...
    /*it('should support complex queries', async () => {
      const result = await query()
        .whereAnd(intersectionQuery => intersectionQuery
          .branch(branchQuery => branchQuery
            .whereOr(
              subQuery => subQuery.whereType('things', 'number'),
              subQuery => subQuery.whereType('things', 'string'),
            )
    ...
    it('should support or queries', async () => {
      const result = await query()
        .whereOr2(unionQuery => unionQuery
    ...
    it('should support and queries', async () => {
      const result = await query()
        .whereAnd(intersectionBuilder => intersectionBuilder
          .branch(branchQuery => branchQuery.whereNotNull('address'))
          .branch(branchQuery => branchQuery
            .whereOr(
              unionQuery => unionQuery.whereType('things', 'string'),
              unionQuery => unionQuery.whereType('things', 'number'),
            )
          )
        )
        .find();
    ...
    it('should support and query with a branch requiring a property to be null', async () => {
      const result = await query()
        .whereAnd(intersectionBuilder => intersectionBuilder
          .branch(branchQuery => branchQuery.whereNotNull('address'))
          .branch(branchQuery => branchQuery.whereType('things', 'null'))
        )
        .find();
