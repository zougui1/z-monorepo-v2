...
import { GetSizeOptions } from './getSize';
import { getOptionalSize } from './getOptionalSize';

describe('getOptionalSize', () => {
  it('should throw an error when the max constraint is less than 0', () => {
    ...
    const getResult = () => getOptionalSize(size, options);

    expect(getResult).toThrowError(/maximum size constraint must be greater than 0/i);
  });

  describe('when the size is neither a number nor a string', () => {
    it('should return undefined when the size is undefined', () => {
      ...
      const result = getOptionalSize(size, options);

      expect(result).toBeUndefined();
    });

    it('should return undefined when the size is NaN', () => {
      const size = NaN;
      const options: GetSizeOptions = {
        max: 69,
        min: 42,
