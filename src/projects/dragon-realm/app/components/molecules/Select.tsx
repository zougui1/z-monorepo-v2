...
const SelectRoot = forwardRef<HTMLSpanElement, SelectProps>(({ label, children, className, classes, helperText, error, ...rest }, ref) => {
  const id = useId();
  const styles = select({ error });

  return (
    <span ref={ref} className={styles.base({ className })}>
      {label && (
        <Label
          htmlFor={id}
...
interface SelectProps extends SelectPrimitiveProps {
  label?: string;
  className?: string;
  classes?: Partial<Record<'label' | 'helperText', string>>;
  helperText?: string;
  error?: boolean;
}
